import { pgTable, text, serial, integer, boolean, decimal, json, timestamp, varchar, primaryKey, real, unique } from "drizzle-orm/pg-core";
...
export const eosUsers = pgTable("eos_users", {
  ...
  eosPermissions: json("eos_permissions").$type<string[]>(), // ['view_rocks', 'edit_issues', 'admin']
...
export const eosChanges = pgTable('eos_changes', {
  ...
  relatedTasks: json('related_tasks').$type<string[]>(),
  metrics: json('metrics').$type<{ before?: number; current?: number; target?: number; unit?: string }>(),
...
export const userFeedback = pgTable('user_feedback_chatbot', {
  ...
  feedbackCategories: json('feedback_categories').$type<string[]>().default([]),
...
// Duplicate type definitions removed - already defined earlier in file
