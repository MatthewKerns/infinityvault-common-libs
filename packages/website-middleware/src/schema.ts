...
export const verificationEvents = brandwebsiteSchema.table("verification_events", {
  ...
  eventMeta: json("event_meta").$type<Record<string, any>>(),
...
export const scratchWinParticipations = pgTable('scratch_win_participations', {
  ...
  stepData: json('step_data').$type<Record<string, any>>().default({}),
...
export const userCartAbandonment = pgTable("user_cart_abandonment", {
  ...
  products: json("products").$type<Array<{productId: number, variantId: number, quantity: number, price: string}>>(),
...
export const userPreferences = pgTable("user_preferences", {
  ...
  favoriteCategories: json("favorite_categories").$type<string[]>(),
  priceAlerts: json("price_alerts").$type<Array<{productId: number, targetPrice: string}>>(),
  wishlist: json("wishlist").$type<number[]>(),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});
...
export const marketingTests = brandwebsiteSchema.table("marketing_tests", {
  ...
  trafficSplit: json("traffic_split").$type<number[]>().notNull(), // e.g., [50, 50] for 50/50 split
...
export const amazonProducts = brandwebsiteSchema.table("amazon_products", {
  ...
  images: json("images").$type<Array<{url: string, type: string, alt?: string}>>(),
  features: json("features").$type<string[]>(),
  specifications: json("specifications").$type<Record<string, string>>(),
  aPlusContent: json("a_plus_content").$type<Record<string, any>>(),
...
export const marketingMaterials = brandwebsiteSchema.table("marketing_materials", {
  ...
  extractedKeywords: json("extracted_keywords").$type<Array<{keyword: string, relevance: number}>>(),
  sentimentScore: decimal("sentiment_score", { precision: 3, scale: 2 }),
  readabilityScore: decimal("readability_score", { precision: 5, scale: 2 }),
  competitiveElements: json("competitive_elements").$type<Array<{element: string, type: string, impact: string}>>(),
...
export const productReviewsAggregated = brandwebsiteSchema.table("product_reviews_aggregated", {
  ...
  extractedIssues: json("extracted_issues").$type<Array<{issue: string, severity: string, frequency: number}>>(),
  extractedFeatures: json("extracted_features").$type<Array<{feature: string, sentiment: string, mentions: number}>>(),
  processedAt: timestamp("processed_at").defaultNow(),
});
...
export const searchQueries = brandwebsiteSchema.table("search_queries", {
  ...
  results: json("results").$type<Array<{asin: string, title: string, price: number, rating: number, rank: number}>>(),
  userId: varchar("user_id", { length: 255 }),
...
export const insertVerificationTicketSchema = createInsertSchema(verificationTickets).omit({
  ...
  orderDate: z.date().refine(
    date => date <= new Date() && date >= new Date(Date.now() - 180 * 24 * 60 * 60 * 1000),
    "Order date must be within the last 180 days"
  ),
...
// ===============================
// MARKETING SPLIT TESTING TYPES
// ===============================

// Type definitions for marketing split testing
...
export interface TestVariant {
  ...
  weight?: number; // For weighted traffic splits
}
...
export interface AmazonListingTestConfig {
  ...
  marketplaces: string[]; // ['US', 'CA', 'UK', etc.]
}
...
// Video marketing specific types
...
export interface VideoTestVariant extends TestVariant {
  ...
  description?: string;
  callToAction?: string;
  hooks?: string[]; // Different opening lines
...
// ===============================
// PRODUCT RESEARCH TYPE EXPORTS
// ===============================

// Product Research core types
...
export type ProductWithReviews = AmazonProduct & {
  ...
  reviewAnalysis: {
    ...
    topFeatures: Array<{feature: string, mentions: number, sentiment: string}>;
  };
};
...
// Search and analysis response types
...
export interface SalesAnalysisResponse {
  ...
  trends: {
    salesVelocity: number; // % change
    priceMovement: number; // % change
    reviewGrowth: number; // % change
  };
}
...
// Utility types for enum constraints
