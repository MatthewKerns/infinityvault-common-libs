...
import { SecurityValidator, PasswordSecurity, SessionSecurity } from '../security';
...
export class AuthCore {
  ...
  async register(
    ...
    try {
      ...
      // Hash password
      const passwordHash = await PasswordSecurity.hashPassword(request.password);
  ...
  async login(
    ...
    try {
      ...
      // Verify password
      const isPasswordValid = await PasswordSecurity.comparePassword(
        request.password,
        user.passwordHash
      );
      if (!isPasswordValid) {
      ...
      if (!user.isVerified) {
        throw new AuthError('Please verify your email before logging in', 'EMAIL_NOT_VERIFIED', 403);
      }
      ...
      const result = await this.repositories.withTransaction(async (repos) => {
        ...
        let rememberMeTokenValue: string | undefined;
  ...
  async createRememberMeToken(
    ...
    if (currentTokenCount >= this.config.rememberMe.maxTokensPerUser) {
      await this.repositories.rememberMe.cleanupExcessTokens(
        userId,
  ...
  async validateRememberMeToken(
    ...
    try {
      ...
      if (!dbToken || !dbToken.isActive) {
        throw new AuthError('Invalid remember me token', 'INVALID_TOKEN', 401);
      }

      // Check if token is expired
      if (dbToken.expiresAt < new Date()) {
        await this.repositories.rememberMe.invalidateToken(tokenHash);
        throw new AuthError('Remember me token expired', 'TOKEN_EXPIRED', 401);
      }
      ...
      const result = await this.repositories.withTransaction(async (repos) => {
        ...
        let newRememberMeToken: string | undefined;
      ...
      throw new AuthError('Remember me validation failed', 'REMEMBER_ME_FAILED', 500);
    }
  }
   ...
   */
  async getUserBySession(sessionToken: string): Promise<BaseUser | null> {
  ...
  async changePassword(
    ...
    try {
      ...
      const isCurrentPasswordValid = await PasswordSecurity.comparePassword(
        currentPassword,
        user.passwordHash
      );
