...
exports.createGoogleOAuthService = createGoogleOAuthService;
...
class GoogleOAuthService {
    constructor(clientId, clientSecret, redirectUri, authCore, repositories, securityValidator, config) {
     ...
     */
    generateAuthUrl(params, context) {
     ...
     */
    async handleCallback(code, state, codeVerifier, context) {
        try {
            ...
            if (!stateData) {
                throw new types_1.SecurityError('Invalid OAuth state', 'INVALID_STATE');
            }
            ...
            if (!tokens.access_token) {
                throw new types_1.AuthError('Failed to obtain access token', 'NO_ACCESS_TOKEN');
            }
            // Get user info from Google
            const userInfo = await this.getUserInfo(tokens.access_token);
            // Validate user info
            const validationResult = types_1.GoogleUserInfoSchema.safeParse(userInfo);
            if (!validationResult.success) {
                throw new types_1.AuthError('Invalid user information from Google', 'INVALID_USER_INFO');
            }
            const validatedUserInfo = validationResult.data;
            // Check email verification
            if (!validatedUserInfo.verified_email) {
                throw new types_1.AuthError('Google account email is not verified', 'UNVERIFIED_EMAIL');
            }
            // Find or create user
            const user = await this.findOrCreateUser(validatedUserInfo, stateData, context);
            // Create session
            const session = await this.authCore.createSession(user.id, {
            ...
            throw new types_1.AuthError('OAuth authentication failed', 'OAUTH_FAILED', 500);
        }
    }
    /**
    ...
    async getUserInfo(accessToken) {
        try {
            ...
            throw new types_1.AuthError('Failed to retrieve user information from Google', 'GOOGLE_API_ERROR');
        }
    }
    /**
     * Find existing user or create new user from Google OAuth
     */
    async findOrCreateUser(googleUserInfo, stateData, context) {
        return await this.repositories.withTransaction(async (repos) => {
            ...
            if (!stateData.createAccount) {
                throw new types_1.AuthError('No account found with this email. Please register first.', 'USER_NOT_FOUND', 404);
            }
    ...
    async refreshAccessToken(refreshToken) {
        try {
            ...
            if (!credentials.access_token) {
                throw new types_1.AuthError('Failed to refresh access token', 'REFRESH_FAILED');
            }
            return {
     ...
     */
    async getUserProfile(accessToken, includeContacts = false) {
        try {
            const userInfo = await this.getUserInfo(accessToken);
            ...
            if (includeContacts) {
                try {
                    const profileResponse = await fetch('https://people.googleapis.com/v1/people/me?personFields=names,emailAddresses,photos', {
                        headers: {
                            'Authorization': `Bearer ${accessToken}`,
                            'Accept': 'application/json'
                        }
            ...
            throw new types_1.AuthError('Failed to retrieve user profile', 'PROFILE_ERROR');
        }
    }
    /**
...
function createGoogleOAuthService(config, authCore, repositories, securityValidator, securityConfig) {
    const service = new GoogleOAuthService(config.clientId, config.clientSecret, config.redirectUri, authCore, repositories, securityValidator, securityConfig);
